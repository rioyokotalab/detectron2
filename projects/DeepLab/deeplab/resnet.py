...
class MYDeepLabStem(CNNBlockBase):
    def __init__(self, in_channels=3, out_channels=128, depth=18, norm="BN", pretrained=False):
        super().__init__(in_channels, out_channels, 4)
        if depth == 18:
            resnet_model = resnet.resnet18(pretrained=pretrained)
        elif depth == 50:
            resnet_model = resnet.resnet50(pretrained=pretrained)
...
def custom_build_resnet_deeplab_backbone(cfg, input_shape):
    """
    Create a custom ResNet instance from config.
    Returns:
